...
class Game:
    def __init__(self, questions_file = "questions.json"):
        try:
            ...
            print(f"File {questions_file} doesn't exist.") #check
            sys.exit(1)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    return False # 之后会做个弹窗
                if self.question_screen:
                # use numbers on the keyboard to control
                ...
                if not self.question_screen and not self.game_over:
                    ...
                    for i, region in enumerate(self.option_areas):
                        if region.collidepoint(event.pos):
                            ...
                            if self.selected_option != -1 or self.timer == 0:
                                self.check_answer()
                                break
                    
        return True

    def handle_click(self, position): # position contains x coordinate and y coordiate
        x, y = position
        if y > 100:
