...
import pygame
import sys
from llmtest import generate_jeopardy_round
...
# game running
class Game:
    def __init__(self, use_llm=True, questions_file="questions.json"):
        # 如果 use_llm=True 或者文件不存在，则调用LLM生成
        if use_llm:
            print("Generating questions using LLM...")
            self.show_loading_screen()   # 显示加载动画
            try:
                all_data = generate_jeopardy_round()
            except Exception as e:
                print(f"LLM generation failed: {e}. Falling back to file.")
            ...
            try:
                ...
                print(f"File {questions_file} not found. Using LLM generation.")
    ...
    def show_loading_screen(self):
        ...
        screen.fill(BLACK)
        font = LARGE
        text = font.render("Generating questions with AI...", True, WHITE)
        text_rect = text.get_rect(center=(WIDTH//2, HEIGHT//2))
        screen.blit(text, text_rect)
        pygame.display.flip()
        # 让画面停留一小段时间，或者等待生成完成（生成函数会阻塞，这里只是刷新一次）
...
if __name__ == "__main__":
    import sys
    use_llm = "--llm" in sys.argv or len(sys.argv) == 1   # 默认使用LLM
